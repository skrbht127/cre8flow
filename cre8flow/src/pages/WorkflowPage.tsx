...
export default function WorkflowPage() {
  ...
  const generateMarkdown = () => {
    const archetypeLabel = ARCHETYPES.find(a => a.key === archetype)?.label ?? archetype
    let content = `# ${workflowName}\nArchetype: ${archetypeLabel} | Generated by Cre8Flow | ${new Date().toISOString().slice(0, 10)}\n\n`
  ...
  <div>
    ...
    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
      {ARCHETYPES.map((a) => (
        <button
          key={a.key}
          onClick={() => setArchetype(a.key as ArchetypeKey)}
          disabled={generating}
          className={`p-3 rounded-xl border text-left transition-all ${generating ? 'opacity-40 cursor-not-allowed' : ''}
            archetype === a.key
